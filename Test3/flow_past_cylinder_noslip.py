...
# VMS-LES FORMULATION
F = idt*dot(v, u)*dx - idt*dot(v, u_n)*dx + nu*0.5 * inner(D(u), D(v)) * dx + inner(dot(u_n,
                                                                                        nabla_grad(u)), v) * dx - p * div(v) * dx - div(u) * q * dx - dot(f1, v)*dx + f2*q*dx
F = F + inner(dot(u_n, nabla_grad(v)), T_M*R_M)*dx - dot(grad(q), T_M*R_M)*dx + dot(div(v), T_C*R_C) * \
    dx + inner(dot(u_n, nabla_grad(v).T), T_M*R_M)*dx - \
