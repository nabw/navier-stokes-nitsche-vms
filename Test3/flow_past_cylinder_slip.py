...
# known data
u_exact = Expression(
    ("(16*2.25*x[1]*x[2]*sin(0.125*pi*t)*(0.41-x[1])*(0.41-x[2]))/(0.41*0.41*0.41*0.41)", "0", "0"), domain=mesh, degree=4, t=0)
f1 = Constant((0, 0, 0))
f2 = Constant(0)
...
p_n = project(p_in, Z.sub(1).collapse(), solver_type="mumps",
              form_compiler_parameters=None)
...
F = F + beta * dot(u, tau1) * dot(v, tau1) * ds(3) + beta * dot(u, tau2) * dot(v, tau2) * ds(3) - nu*dot(D(u)*dot(v, n)*n, n)*ds(3) - \
    nu*dot(D(v)*dot(u, n)*n, n)*ds(3) + p*dot(v, n)*ds(3) + q*dot(u, n) * \
    ds(3) + gamma * Constant(1.0 / h) * dot(u, n) * dot(v, n) * ds(3)
...
F = F + nu*dot(D(T_M*R_M)*dot(v, n)*n, n)*ds(3) + nu*dot(D(v)*dot(T_M*R_M, n)
                                                         * n, n)*ds(3) - T_C*R_C*dot(v, n)*ds(3) - q*dot(T_M*R_M, n)*ds(3)
