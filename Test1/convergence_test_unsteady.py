...
# known data
u_exact = Expression(('sin(pi*x[0]-0.7)*sin(pi*x[1]+0.2)*cos(t)',
                     'cos(pi*x[0]-0.7)*cos(pi*x[1]+0.2)*cos(t)'), domain=mesh, degree=5, t=0)
p_exact = Expression(
    ('cos(t)*(sin(x[0])*cos(x[1])+(cos(1)-1)*sin(1))'), domain=mesh, degree=5, t=0)
u_exd = Expression(('-sin(pi*x[0]-0.7)*sin(pi*x[1]+0.2)*sin(t)',
                   '-cos(pi*x[0]-0.7)*cos(pi*x[1]+0.2)*sin(t)'), domain=mesh, degree=5, t=0)
...
F = F + beta * dot(u, tau) * dot(v, tau) * ds(1) - nu*dot(D(u)*dot(v, n)*n, n)*ds(1) - nu*dot(D(v)*dot(u, n)*n, n)*ds(1) + p*dot(v, n)*ds(1) + q*dot(u, n)*ds(1) + gamma * \
    Constant(1.0 / h) * dot(u, n) * dot(v, n) * ds(1) - ss*dot(v, tau)*ds(1) + nu * \
    dot(D(v)*H*n, n)*ds(1) - q*H*ds(1) - gamma * \
    Constant(1.0 / h) * H * dot(v, n) * ds(1)

F = F + inner(grad(v)*u_n, T_M*R_M)*dx - dot(grad(q), T_M*R_M)*dx + dot(div(v), T_C*R_C)*dx + inner((grad(v).T)*u_n, T_M*R_M)*dx - inner(grad(v), outer(T_M *
                                                                                                                                                        r_M, T_M*R_M))*dx - T_C*R_C*dot(v, n)*ds(1) - q*dot(T_M*R_M, n)*ds(1) + nu*dot(D(T_M*R_M)*dot(v, n)*n, n)*ds(1) + nu*dot(D(v)*dot(T_M*R_M, n)*n, n)*ds(1)
...
for z in range(num_steps):
    ...
    error_H1 = abs(assemble(dot(u-u_exact, u-u_exact)*dx +
                   inner(grad(u)-grad(u_exact), grad(u)-grad(u_exact))*dx))
