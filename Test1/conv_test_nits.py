...
for iterAdapt in range(nmax): 
   ...
   # Analytical solution
    u_exac = Expression(('2.0*x[1]*(1.0 -x[0]*x[0])','-2.0*x[0]*(1.0-x[1]*x[1])'), domain=mesh, degree = 3)
  ...
  # Data:
    ...
    # NITSCHE FORMULATION
    F   = (0.5*nu*inner(epsilon(u), epsilon(v)) - div(v)*p - q*div(u) + inner(grad(u)*u, v))*dx + rho*q*dx + lamda*p*dx - inner(f,v)*dx
    F   =  F + beta * u_t(u) * u_t(v) * ds(1) -nu*dot(epsilon(u)*u_n(v)*n, n)*ds(1)-nu*dot(epsilon(v)*u_n(u)*n, n)*ds(1) + p*u_n(v)*ds(1) +q*u_n(u)*ds(1) + gamma*u_n(u)*u_n(v)*ds(1) - nu*dot(epsilon(v)*g*n, n)*ds(1)+q*g*ds(1) + gamma*g*u_n(v)*ds(1) + ss*u_t(v)*ds(1)
    ...
    # Error's computation:
    error_p_L2 = 0.0;  error_u_L2 = 0.0; error_u_sH1 = 0.0; error_slip = 0.0
    ...
    error_u_sH1 = errornorm(u_exac,u, norm_type="H10",degree_rise=6, mesh=mesh)
    error_slip  = sqrt(abs(assemble(u_n(u)*(u_n(u))*ds(1))))                     # ||u.n -g||_{0,\Gamma_s}

    h_mesh.append(mesh.hmax()); ep_L2.append(error_p_L2); eu_L2.append(error_u_L2); eu_sH1.append(error_u_sH1)

    O_p_L2  = 0.0; O_u_L2 = 0.0; O_u_sH1  = 0.0
    # h    dof     Nelem     e_p     e_u     error  Eta  Effectivity
    if iterAdapt > 0:
        O_p_L2  = ln(ep_L2[iterAdapt]/ep_L2[iterAdapt-1]) /ln(h_mesh[iterAdapt]/h_mesh[iterAdapt-1])     #
        O_u_L2  = ln(eu_L2[iterAdapt]/eu_L2[iterAdapt-1]) /ln(h_mesh[iterAdapt]/h_mesh[iterAdapt-1])     # convergence orders
        O_u_sH1 = ln(eu_sH1[iterAdapt]/eu_sH1[iterAdapt-1]) /ln(h_mesh[iterAdapt]/h_mesh[iterAdapt-1])   #
