...
F = F + beta * dot(u, tau) * dot(v, tau) * ds(1)- Constant(nu)*dot(D(u)*dot(v,n)*n, n)*ds(1)- Constant(nu)*dot(D(v)*dot(u,n)*n, n)*ds(1)  + Constant(gamma) * Constant(1.0 / h) * dot(u, n) * dot(v, n) * ds(1) + p*dot(v,n)*ds(1) + q*dot(u,n)*ds(1)
...
F = F + nu*dot(D(T_M*R_M)*dot(v,n)*n, n)*ds(1) +nu*dot(D(v)*dot(T_M*R_M,n)*n, n)*ds(1) - T_C*R_C*dot(v,n)*ds(1) - q*dot(T_M*R_M,n)*ds(1) 
