...
# NITSCHE FORMULATION
F = Constant(nu)*0.5 * inner(D(u), D(v)) * dx + inner(grad(u)*u, v) * dx - p * \
    div(v) * dx - div(u) * q * dx + rho*q * \
    dx + lamda*p*dx - dot(f, v)*dx + g*q*dx
F = F + Constant(beta) * dot(u, tau) * dot(v, tau) * ds(1) - Constant(nu)*dot(D(u)*dot(v, n)*n, n)*ds(1) - Constant(nu)*dot(D(v) *
                                                                                                                            dot(u, n)*n, n)*ds(1) + Constant(gamma) * Constant(1.0 / h) * dot(u, n) * dot(v, n) * ds(1) + p*dot(v, n)*ds(1) + q*dot(u, n)*ds(1)
